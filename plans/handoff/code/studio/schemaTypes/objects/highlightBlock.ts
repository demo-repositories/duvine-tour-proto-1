...
export const highlightBlockType = defineType({
  ...
  fields: [
    defineField({
      ...
      title: 'Category',
      type: 'string',
      description: 'What kind of callouts these are. New categories can be added in code without breaking existing content.',
      options: {
        list: Object.entries(CATEGORY_LABELS).map(([value, title]) => ({value, title})),
      },
    ...
    defineField({
      ...
      type: 'array',
      of: [{type: 'string'}],
      description: 'Short bulleted entries — one per line. Each item is a single sentence or phrase. Example for Eat — Truffle hunting and tasting in Chianti',
