...
export const itineraryDayType = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      type: 'image',
      description: 'The featured image for this day\'s section. Landscape orientation works best.',
      options: {hotspot: true},
      fields: [
    ...
    defineField({
      ...
      type: 'object',
      description: 'How far guests ride this day. Both metric and imperial — both are shown to guests based on their region.',
      fields: [
        defineField({name: 'kilometers', type: 'number', title: 'Kilometers', validation: (R) => R.min(0).max(250)}),
        defineField({name: 'miles', type: 'number', title: 'Miles', validation: (R) => R.min(0).max(160)}),
      ],
    }),
    defineField({
      name: 'elevation',
