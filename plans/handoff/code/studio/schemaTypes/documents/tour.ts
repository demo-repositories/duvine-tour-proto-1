...
export const tourType = defineType({
  ...
  fields: [
      ...
      validation: (Rule) =>
        Rule.required()
          ...
          .custom((itinerary, context) => {
            const duration = (context.document as {durationDays?: number} | undefined)
              ?.durationDays
            if (!itinerary || !Array.isArray(itinerary) || duration == null) return true
    ...
    defineField({
      ...
      group: 'practical',
      validation: (Rule) => Rule.min(3).error('Add at least three detail sections.'),
    }),
      ...
      of: [
        defineArrayMember({
          ...
          options: {
            ...
            filterParams: {
              // ^.difficulty references the parent (tour) document's difficulty field
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.custom(async (featured, context) => {
          ...
          if (others.length >= 6) {
            const names = others.map((o) => o.title).filter(Boolean).join(', ')
    ...
    defineField({
      ...
      fields: [
        ...
        defineField({
          ...
          type: 'text',
          description: 'One- to two-sentence description for search results. About 155 characters.',
